...
class HyenaPreTrainedModel(PreTrainedModel):
...
class HeynaModel(HyenaPreTrainedModel):
    def __init__(
        self,
        config: HyenaConfig
    ):
        ...
        self.config = config
        self.backbone = HyenaBackbone(config)

        # Initialize weights and apply final processing
        self.post_init()

    def forward(self, inputs_embeds, output_hidden_states=None):
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )

        hidden_states, all_hidden_states = self.backbone(inputs_embeds, output_hidden_states=output_hidden_states)
        if output_hidden_states:
            return hidden_states, all_hidden_states
        else:
